    ...
    <q-card style="width: 800px; max-width: 90vw">
      <q-card-section>
        ...
        <div v-show="!finished && !langselect">
          ...
          <!-- Answers -->
          <q-card class="bg-green-2 q-ma-md" v-if="currentQuestion.type !== 'textOnly'">

            <!-- freetext -->
            <q-input v-if="currentQuestion.type === 'freetext'" class="q-ma-sm" v-model="currentAnswerFreeText"
              align="center" type="textarea" rows="3" clearable />

            <!-- number -->
            <q-input v-if="currentQuestion.type === 'number'" class="q-ma-sm" v-model.number="currentAnswerNumber"
              align="center" type="number" :rules="[
            ...
            <div v-if="currentQuestion.type === 'slider'" class="text-center">
              <div class="row justify-center">
                <q-slider v-model.number="currentAnswerNumber" :min="currentQuestion.min" :max="currentQuestion.max"
                  :vertical="currentQuestion.vertical" :reverse="currentQuestion.vertical" />
              </div>
            </div>
            ...
            <div class="q-ma-sm" v-if="currentQuestion.type === 'singleChoice'">
              <div v-for="(answerChoice, index) in currentQuestion.answerChoices" :key="index">
                <q-radio v-model="currentAnswerSingleChoice" :val="answerChoice.id"
                  :label="answerChoice.text[language]" />
                <q-input v-show="answerChoice.includeFreeText" :disable="currentAnswerSingleChoice !== answerChoice.id"
                  v-model="currentAnswerSingleChoiceFreeText" type="textarea" :label="answerChoice.text[language]"
                  rows="1" clearable />
              </div>
            </div>
            ...
            <div class="q-ma-sm" v-if="currentQuestion.type === 'multiChoice'">
              <div v-for="(answerChoice, index) in currentQuestion.answerChoices" :key="index">
                <q-checkbox v-model="currentAnswerMultiChoice" :val="answerChoice.id"
                  :label="answerChoice.text[language]" />
                <q-input v-show="answerChoice.includeFreeText"
                  :disable="!currentAnswerMultiChoice.includes(answerChoice.id)"
                  v-model="currentAnswerMultiChoiceFreeText[index]" type="textarea" :label="answerChoice.text[language]"
                  rows="1" clearable />
              </div>
            </div>
          </q-card>
    ...
    next () {
      ...
      if (type === 'freetext' || type === 'number' || type === 'multiChoice') {
        ...
        if (this.currentAnswerSingleChoice) {
          const anschoice = this.currentQuestion.answerChoices.find((a) => {
      ...
      if (nextQId === 'ENDFORM') {
        ...
        const nextQIdx = this.form.questions.findIndex((q) => {
          return q.id === nextQId
        })
        this.currentAnswerSingleChoice = undefined
        this.currentAnswerSingleChoiceFreeText = undefined
